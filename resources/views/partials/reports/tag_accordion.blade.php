...
<div id="tag_tickets_accordion" class="card-body pt-0">
    @foreach($groupedTickets as $index => $tagData)
        <div class="py-0">
            <div class="py-5 d-flex flex-stack flex-wrap">
                <div class="d-flex align-items-center collapsible collapsed rotate"
                     data-bs-toggle="collapse"
                     href="#tag_tickets_accordion_{{$index}}"
                     role="button"
                     aria-expanded="false"
                     aria-controls="tag_tickets_accordion_{{$index}}">
                    ...
                    <div class="me-3">
                        <div class="d-flex align-items-center">
                            ...
                            <div class="badge badge-light-primary ms-5">
                                {{ $tagData['tickets']->count() }}
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div id="tag_tickets_accordion_{{$index}}" class="collapse fs-6 ps-10"
                 data-bs-parent="#tag_tickets_accordion"
            >
                <table class="table align-middle table-hover text-center table-row-dashed fs-6 gy-5">
                    ...
                    <tbody class="fw-semibold text-gray-600">
                        @foreach ($tagData['tickets'] as $ticket)
                            <tr class="position_row_{{$ticket->id}}">
                                ...
                                <td class="d-flex align-items-center border-bottom-0">
                                    ...
                                    <div class="d-flex flex-column">
                                        <a href="{{route('cabinet.users.show', $ticket->creator)}}" target="_blank" class="text-gray-800 text-hover-primary mb-1" target="_blank">
                                            {{$ticket->creator->name}}
                                        </a>
