...
class TicketStatusUpdatedNotification extends Notification implements ShouldQueue
{
    ...
    public function toMail($notifiable): MailMessage
    {
        return (new MailMessage)
            ...
            ->line('Пользователь ' . $this->ticketHistory->user->name . ' изменил статус тикета #' . $this->ticketHistory->ticket->id . ' на "' . $this->ticketHistory->status->label() . '"')            ->action('Посмотреть тикет', url('/cabinet/tickets/' . $this->ticketHistory->ticket->id));
